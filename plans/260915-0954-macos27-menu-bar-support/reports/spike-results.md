...
- S4/S5: the seeded preferred position never produced a table key for the new item within 2 seconds, so the legacy seed is not honored. After 3 seconds at length 0 the item returned to the same Accessibility x position; the logged `kept=false` reflects only the missing table key. Cleanup succeeded.
...
- S9: pressing an off-screen third-party item opened its menu at the left edge of the main display. The Accessibility call returned -25204 (cannot complete) while the menu was tracking, so success is judged by the opened menu, not the return code.
